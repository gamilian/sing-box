...
>
>2024.04.12 v1.2.0 1. Add Cloudflare Argo Tunnel, so that 10 protocols, including the transport mode of ws, no longer need to bring our own domain; 2. Cloudflare Argo Tunnel supports try, Json and Token methods. Use of [sb -t] online switching; 3. Cloudflare Argo Tunnel switch is [sb -a], and the Sing-box switch is changed from [sb -o] to [sb -s]; 4. If Json or Token Argo is used, the subscription address is the domain name; 5. For details: https://github.com/gamilian/sing-box; 1. 增加 Cloudflare Argo Tunnel，让包括传输方式为ws在内的10个协议均不再需要自带域名; 2. Cloudflare Argo Tunnel 支持临时、Json 和 Token 方式，支持使用 [sb -t] 在线切换; 3.  Cloudflare Argo Tunnel 开关为 [sb -a]，Sing-box 开关从 [sb -o] 更换为 [sb -s]; 4. 若使用 Json 或者 Token 固定域名 Argo，则订阅地址则使用该域名; 5. 详细参考: https://github.com/gamilian/sing-box
>
>2024.04.01 sing-box + argo container version is newly launched, for details: https://github.com/gamilian/sing-box; sing-box 全家桶 + argo 容器版本全新上线，详细参考: https://github.com/gamilian/sing-box
>
>2024.03.27 v1.1.11 Add two non-interactive installation modes: 1. pass parameter; 2.kv file, for details: https://github.com/gamilian/sing-box; 增加两个的无交互安装模式: 1. 传参；2.kv 文件，详细参考: https://github.com/gamilian/sing-box
>
...
* 节点信息输出到 V2rayN / Clash Verge / 小火箭 / Surge / Nekobox / Sing-box (SFI, SFA, SFM)，订阅自动适配客户端，一个订阅 url 走天下
...
* 首次运行
```
bash <(wget -qO- https://raw.githubusercontent.com/gamilian/sing-box/main/sing-box.sh)
```
...
#### 中文
```
bash <(wget -qO- https://raw.githubusercontent.com/gamilian/sing-box/main/sing-box.sh) -l
```

#### 英文
```
bash <(wget -qO- https://raw.githubusercontent.com/gamilian/sing-box/main/sing-box.sh) -k
```

### 方式2. KV 配置文件，内容参照本库里的 config.conf
```
bash <(wget -qO- https://raw.githubusercontent.com/gamilian/sing-box/main/sing-box.sh) -f config.conf
```
...
|   |-- qr                                   # Nekoray / V2rayN / Shadowrock / Surge 订阅二维码
|   |-- shadowrocket                         # Shadowrock 订阅文件
|   |-- surge                                # Surge 托管配置订阅文件
|   |-- surge-proxies                        # Surge policy-path 外部策略列表
